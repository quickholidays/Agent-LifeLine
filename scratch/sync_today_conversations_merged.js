  ...
  while (attempts < 3) {
    ...
    try {
      ...
      if (response.status === 429) {
        console.warn(`[GHL Rate Limit 429] Waiting 5 seconds...`);
        await sleep(5000);
        continue;
      }
...
async function run() {
  console.log(`=== RUNNING TODAY'S CONVERSATION SYNC & MERGE FOR DATE: ${targetDateStr} ===\n`);
  ...
  console.log(`Successfully loaded existing report. Initial messages: ${(report.ghl_outbound_messages || []).length}`);
  ...
  console.log(`\nFetched ${fetchedMessages.length} GHL outbound SMS messages.`);

  // 4. Merge into existing report without duplicates
  const existingMsgs = report.ghl_outbound_messages || report.ghlMessages || [];
  ...
  console.log(`Merged results. Added ${newMessagesAdded} brand new messages. Total daily GHL messages: ${existingMsgs.length}`);

  // 5. Upload updated daily backup to GitHub
  console.log(`Uploading merged daily report back to GitHub...`);
  const putPayload = {
    message: `chore: sync and merge live GHL conversations for ${targetDateStr} (added ${newMessagesAdded} new)`,
  ...
  if (putRes.status === 200 || putRes.status === 201) {
    console.log(`✅ SUCCESS! Merged GHL conversations into today's CSV-compiled report on GitHub.`);
  } else {
    console.error(`❌ FAILED to upload merged daily report: ${putRes.body}`);
  }
}

run().catch((e) => console.error("Sync and merge run failed:", e.message));
