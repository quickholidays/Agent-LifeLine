...
async function runDynamicSync() {
  const targetDateStr = "2026-07-29"; // Hardcoded Yesterday's date
  console.log(`=== RUNNING GHL CONVERSATION SYNC & MERGE FOR YESTERDAY: ${targetDateStr} ===\n`);
  ...
  // 3. Fetch Conversations active today/yesterday from GHL API
  ...
  while (hasMore && pageCount < 100) { // Check up to 100 pages for historical sync
    pageCount++;
    const params = {
    ...
    for (const c of conversations) {
      ...
      // If conversation has activity on target date
        ...
        if (Array.isArray(messagesList)) {
          for (const m of messagesList) {
            ...
            if (msgDateStr === targetDateStr && isOutbound && !isCall && !isEmail && !isOpportunityLog && !isPlaceholderSms) {
              ...
              const baseContactName = c.fullName || "GHL Contact";
              const contactName = contactId ? `${baseContactName} (${contactId})` : baseContactName;

              // Tier 2: Lookup via Contact Assignment on GHL
              if ((!agentName || agentName === "Unassigned") && baseContactName && baseContactName.toLowerCase() !== "ghl contact") {
                if (contactCache[baseContactName] === undefined) {
                  const assignedTo = await findContactAssignment(baseContactName);
                  if (assignedTo && userMap[assignedTo]) {
                    contactCache[baseContactName] = userMap[assignedTo];
                  } else {
                    contactCache[baseContactName] = null;
                  }
                  await sleep(150);
                }
                if (contactCache[baseContactName]) {
                  agentName = contactCache[baseContactName];
                }
              }

              // Tier 3: Lookup via CSV segmentations
              if ((!agentName || agentName === "Unassigned") && baseContactName) {
                const cleanName = baseContactName.trim().toLowerCase();
      ...
      } else if (new Date(lastMsgDate) < new Date(new Date().setHours(0,0,0,0) - 24 * 60 * 60 * 1000 * 5)) {
        // Stop paging if conversation is older than 5 days ago (since we are sync'ing yesterday)
        foundOlder = true;
      }
    }
  ...
  console.log(`\nFetched ${fetchedMessages.length} GHL outbound messages for yesterday (${targetDateStr}).`);
  ...
  const putPayload = {
    message: `chore: automatic GHL sync & merge for yesterday ${targetDateStr} (added ${newMessagesAdded} new)`,
  ...
  if (putRes.status === 200 || putRes.status === 201) {
    console.log(`✅ SUCCESS! Synced and merged yesterday's GHL conversations into report on GitHub.`);
  } else {
